...
void CpuRegistersComponent::paintListBoxItem(int rowNumber, Graphics& g, int width, int height, bool rowIsSelected)
{
	...
	{
	case 0:
		FormatRegisterPairString("AF", registers_state_.af, previous_registers_state_.af, compute_diff_).draw(g, Rectangle<int>{ width, height }.toFloat());
		break;
	case 1:
		FormatRegisterPairString("BC", registers_state_.bc, previous_registers_state_.bc, compute_diff_).draw(g, Rectangle<int>{ width, height }.toFloat());
		break;
	case 2:
		FormatRegisterPairString("DE", registers_state_.de, previous_registers_state_.de, compute_diff_).draw(g, Rectangle<int>{ width, height }.toFloat());
		break;
	case 3:
		FormatRegisterPairString("HL", registers_state_.hl, previous_registers_state_.hl, compute_diff_).draw(g, Rectangle<int>{ width, height }.toFloat());
		break;
	case 4:
		Format16bitRegisterString("SP", registers_state_.sp, previous_registers_state_.sp, compute_diff_).draw(g, Rectangle<int>{ width, height }.toFloat());
		break;
	case 5:
		Format16bitRegisterString("PC", registers_state_.pc, previous_registers_state_.pc, compute_diff_).draw(g, Rectangle<int>{ width, height }.toFloat());
		break;
	}
}

AttributedString CpuRegistersComponent::FormatRegisterPairString(std::string register_name, const RegisterPair &register_pair_value, const RegisterPair &register_pair_previous_value, bool compute_diff)
{
	...
	formatted_string.append(high.str(), (compute_diff && (register_pair_value.GetHigh() != register_pair_previous_value.GetHigh())) ? Colours::red : Colours::black);

	std::stringstream low;
	low << std::uppercase << std::setfill('0') << std::setw(2) << std::hex << static_cast<int>(register_pair_value.GetLow());
	formatted_string.append(low.str(), (compute_diff && (register_pair_value.GetLow() != register_pair_previous_value.GetLow())) ? Colours::red : Colours::black);
...
AttributedString CpuRegistersComponent::Format16bitRegisterString(std::string register_name, uint16_t register_value, uint16_t register_previous_value, bool compute_diff)
{
	AttributedString formatted_string;
	formatted_string.append(register_name);
	formatted_string.append(": ");
	...
	formatted_string.append(value.str(), (compute_diff && (register_value != register_previous_value)) ? Colours::red : Colours::black);
...
void CpuRegistersComponent::UpdateRegistersState(CPU::Registers registers_state, CPU::Flags flags_state, bool compute_diff)
{
	compute_diff_ = compute_diff;

	previous_registers_state_ = registers_state_;
	registers_state_ = registers_state;

	registers_list_.updateContent();
	registers_list_.repaint();
