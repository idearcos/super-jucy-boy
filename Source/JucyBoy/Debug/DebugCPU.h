...
public:
	...
	public:
		virtual ~Listener() {}
		virtual void OnBreakpointHit(const Memory::Address &/*breakpoint*/) {}
	...
	// Breakpoints
	inline void AddBreakpoint(const Memory::Address &address) { breakpoints_.insert(address); } //TODO: allow only when not running!
	inline void RemoveBreakpoint(const Memory::Address &address) { breakpoints_.erase(address); } //TODO: allow only when not running!
...
private:
	...
	bool IsReadWatchpointHit(const Memory::Address &address) const;
	bool IsWriteWatchpointHit(const Memory::Address &address) const;

	// Listener notification
	void NotifyBreakpointHit(const Memory::Address &breakpoint) const;
